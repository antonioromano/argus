...
| B7 | Scroll to bottom on typing | `scrollOnUserInput: true` | SwiftTerm `send(data:)` already calls `ensureCaretIsVisible()`; **only Argus's own Shift+Enter path bypasses it** (calls `onInput` directly). | Route Shift+Enter (and B1's keys) through `terminalView.send(data:)`. |
