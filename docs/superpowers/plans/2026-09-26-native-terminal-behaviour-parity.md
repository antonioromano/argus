...
| B5 | Wheel speed | `scrollSensitivity: 3`, `fastScrollSensitivity: 10` with Option (xterm's default `fastScrollModifier: 'alt'`); `Viewport._applyScrollModifier` multiplies by BOTH when the modifier is held (`amount * fastScrollSensitivity * scrollSensitivity`), i.e. 30, not 10. | SwiftTerm `scrollSensitivity = 1`, no fast mode. | Sensitivity 3, and 30 (not 10) while Option is held — corrected in final review round 2, which had this at 10. |
